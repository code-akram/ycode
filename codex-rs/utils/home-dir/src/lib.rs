...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    ...
    find_codex_home_from_env(codex_home_env.as_deref(), home_dir())
}
...
fn find_codex_home_from_env(
    ...
    match codex_home_env.filter(|val| !val.is_empty()) {
        Some(val) => {
...
mod tests {
    ...
    fn find_codex_home_env_valid_directory_canonicalizes() {
        let temp_home = TempDir::new().expect("temp home");
        let default_home = TempDir::new().expect("default temp home");
        ...
        let resolved =
            find_codex_home_from_env(Some(temp_str), Some(default_home.path().to_path_buf()))
    ...
    #[test]
    fn find_codex_home_without_env_uses_ycode_under_supplied_home() {
        let temp_home = TempDir::new().expect("temp home");
        ...
        let expected = temp_home.path().join(".ycode");
        let expected = AbsolutePathBuf::from_absolute_path(expected).expect("absolute home");
        assert_eq!(resolved, expected);
    }

    #[test]
    fn find_codex_home_with_empty_env_uses_ycode_under_supplied_home() {
        let temp_home = TempDir::new().expect("temp home");
        let resolved = find_codex_home_from_env(Some(""), Some(temp_home.path().to_path_buf()))
            .expect("default CODEX_HOME");
        let expected = temp_home.path().join(".ycode");
        let expected = AbsolutePathBuf::from_absolute_path(expected).expect("absolute home");
        assert_eq!(resolved, expected);
