...
impl SqliteConfig {
    ...
    pub(super) async fn open_state_db(&self, migrator: &Migrator) -> anyhow::Result<SqlitePool> {
        ...
        self.open_runtime_db(STATE_DB, migrator).await
    }

    pub(super) async fn open_logs_db(&self, migrator: &Migrator) -> anyhow::Result<SqlitePool> {
        self.open_runtime_db(LOGS_DB, migrator).await
    }

    pub(super) async fn open_goals_db(&self, migrator: &Migrator) -> anyhow::Result<SqlitePool> {
        self.open_runtime_db(GOALS_DB, migrator).await
    }

    pub(super) async fn open_memories_db(&self, migrator: &Migrator) -> anyhow::Result<SqlitePool> {
        self.open_runtime_db(MEMORIES_DB, migrator).await
    }

    pub(super) async fn open_queue_db(&self, migrator: &Migrator) -> anyhow::Result<SqlitePool> {
        self.open_runtime_db(QUEUE_DB, migrator).await
    }
    ...
    pub(super) async fn open_thread_history_db(
        ...
        self.open_runtime_db(THREAD_HISTORY_DB, migrator).await
    }
