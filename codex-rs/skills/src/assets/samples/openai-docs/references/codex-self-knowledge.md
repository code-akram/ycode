...
- Project `.codex/config.toml`: settings for a trusted repository, including model and reasoning defaults.
...
When a page-specific citation helps, useful official anchors include `concepts/customization#agents-guidance`, `concepts/customization#skills`, and `config-reference#configtoml`.
