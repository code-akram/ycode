...
fn collect_mentions(
    ...
    collect_explicit_skill_mentions(inputs, &loaded_skills)
}
...
fn collect_explicit_skill_mentions_text_respects_skill_order() {
    ...
    let selected = collect_mentions(&inputs, &skills, &HashSet::new());
...
fn collect_explicit_skill_mentions_accepts_structured_discovery_path() {
    ...
    let selected = collect_explicit_skill_mentions(&inputs, &loaded_skills);
...
fn collect_explicit_skill_mentions_skips_disabled_structured_and_blocks_plain_fallback() {
    ...
    let selected = collect_mentions(&inputs, &skills, &disabled);
