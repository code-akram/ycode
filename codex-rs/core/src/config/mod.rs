...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.ycode/config.toml` snippet:
    ///
    /// ```toml
    ...
    /// Directory containing all Codex state (defaults to `~/.ycode` but can be
...
impl Config {
    ...
    /// in [ConfigToml], such as `cwd`, `codex_self_exe`, and `main_execve_wrapper_exe`.
