...
impl Session {
    ...
    pub(crate) async fn new(
        ...
        let auth_fut: Pin<Box<dyn Future<Output = Option<CodexAuth>> + Send>> =
            Box::pin(async move { auth_manager_clone.auth().await });
        let auth_fut = auth_fut.instrument(info_span!(
            "session_init.auth",
