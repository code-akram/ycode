...
pub(crate) async fn run_turn(
    ...
    let Some(injection_items) = build_skill_injections_for_turn(
        &sess,
...
#[instrument(level = "trace", skip_all)]
async fn build_skill_injections_for_turn(
    sess: &Arc<Session>,
    ...
    let mut injection_items = skill_injections
        .iter()
        .filter(|skill| {
            injected_host_skill_prompts
                .as_ref()
                .is_none_or(|injected| !injected.contains_path(&skill.path))
        })
...
pub(crate) fn get_last_assistant_message_from_turn(responses: &[ResponseItem]) -> Option<String> {
    for item in responses.iter().rev() {
        if let Some(message) = last_assistant_message_from_item(item) {
            return Some(message);
        }
    }
    None
