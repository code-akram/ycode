...
fn render_plugins_section_keeps_plugin_usage_guidance_without_listing_plugins() {
    ...
    let expected = "<plugins_instructions>\n## Plugins\nA plugin is a local bundle of skills and supporting metadata.\n### How to use plugins\n- Skill naming: If a plugin contributes skills, those skill entries are prefixed with `plugin_name:` in the Skills list.\n- Trigger rules: If the user explicitly names a plugin, prefer capabilities associated with that plugin for that turn.\n- Relationship to capabilities: Plugins are not invoked directly. Use their underlying skills to help solve the task.\n- Relevance: Determine what a plugin can help with from explicit user mention or from plugin-associated skills exposed elsewhere in this turn.\n- Missing/blocked: If the user requests a plugin that does not have relevant callable capabilities for the task, say so briefly and continue with the best fallback.\n</plugins_instructions>";
...
fn explicit_plugin_instructions_are_bounded() {
    let rendered = render_explicit_plugin_instructions(&PluginCapabilitySummary {
        config_name: "sample@test".to_string(),
        display_name: "x".repeat(MAX_EXPLICIT_PLUGIN_INSTRUCTIONS_BYTES * 2),
        has_skills: true,
        ..PluginCapabilitySummary::default()
    })
    .expect("skill capability should render");
