...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn exec_command(
        ...
        let process = self.open_session(&request, cwd.clone(), context).await;
        ...
        let (response_process_id, exit_code) = if process_started_alive {
            match self.refresh_process_state(process_id).await {
                ...
                ProcessStatus::Exited { exit_code, .. } => (None, exit_code),
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        let spawn_result = codex_utils_pty::spawn_process(codex_utils_pty::SpawnRequest {
    ...
    pub(super) async fn open_session(
        ...
        orchestrator
            ...
            .map_err(|err| match err {
                ToolError::Codex(err) => UnifiedExecError::create_process(format!("{err:?}")),
