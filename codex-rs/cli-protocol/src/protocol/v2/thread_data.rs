...
impl From<CoreSessionSource> for SessionSource {
    fn from(value: CoreSessionSource) -> Self {
        match value {
            ...
            CoreSessionSource::Mcp => SessionSource::CliRuntime,
            CoreSessionSource::Custom(source) => SessionSource::Custom(source),
            // We do not want to render those at the cli-runtime level.
...
impl From<SessionSource> for CoreSessionSource {
    fn from(value: SessionSource) -> Self {
        match value {
            ...
            SessionSource::CliRuntime => CoreSessionSource::Mcp,
...
/// Extra cli-runtime data for a thread.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase", export_to = "v2/")]
...
pub struct Thread {
    ...
    /// Origin of the thread (CLI, VSCode, codex exec, codex cli-runtime, etc.).
...
pub enum TurnItemsView {
    ...
    Summary,
    /// `items` contains every ThreadItem available from persisted cli-runtime history for this turn.
    #[default]
    Full,
}
