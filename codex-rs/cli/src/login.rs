...
async fn clear_existing_auth_before_login(codex_home: &Path, auth_route_config: &AuthRouteConfig) {
    if let Err(err) = logout_with_revoke(codex_home, auth_route_config).await {
...
pub async fn login_with_chatgpt(
    ...
    clear_existing_auth_before_login(&codex_home, &auth_route_config).await;
...
pub async fn run_login_with_device_code(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    clear_existing_auth_before_login(&config.codex_home, &auth_route_config).await;
    let effective_chatgpt_workspaces = config.auth_config().effective_chatgpt_workspaces();
    let opts = ServerOptions::new(
        config.codex_home.to_path_buf(),
        CLIENT_ID.to_string(),
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout_with_revoke(&config.codex_home, &auth_route_config).await {
        Ok(true) => {
...
mod tests {
    ...
    async fn clears_existing_auth_before_login() {
        ...
        std::fs::write(codex_home.path().join("auth.json"), "{}").expect("save existing auth");
        ...
        let auth = load_auth_dot_json(codex_home.path()).expect("load auth after cleanup");
        assert_eq!(auth, None);
    }
