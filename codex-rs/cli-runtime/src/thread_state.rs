...
pub(crate) struct PendingThreadResumeRequest {
    ...
    pub(crate) thread_summary: codex_cli_protocol::Thread,
    pub(crate) emit_thread_goal_update: bool,
    pub(crate) thread_goal_state_db: Option<StateDbHandle>,
    pub(crate) include_turns: bool,
    pub(crate) initial_turns_page: Option<codex_cli_protocol::ThreadResumeInitialTurnsPageParams>,
    pub(crate) paginated_turns: Option<Vec<Turn>>,
    pub(crate) paginated_initial_turns_page: Option<codex_cli_protocol::TurnsPage>,
    pub(crate) paginated_initial_turns_page_with_active_slot: Option<codex_cli_protocol::TurnsPage>,
...
pub(crate) enum ThreadListenerCommand {
    ...
    // EmitThreadGoalCleared is used to order cli-runtime goal clears with running-thread resume responses.
    ...
    pub(crate) async fn clear_all_listeners(&self) {
        ...
        for (thread_id, thread_state) in thread_states {
            ...
            tracing::debug!(
                ...
                had_listener = thread_state.cancel_tx.is_some(),
                had_active_turn = thread_state.active_turn_snapshot().is_some(),
                "clearing thread listener during cli-runtime shutdown"
            );
            thread_state.clear_listener();
        }
    }
