use codex_cli_protocol::ConfigLayer as ApiConfigLayer;
use codex_cli_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
...
/// Converts a config-layer source owned by `codex-config` into the cli-runtime wire type owned by
/// `codex-cli-protocol`.
///
/// The types stay separate so cli-runtime protocol ownership does not leak into the config domain
...
/// Converts config-layer metadata owned by `codex-config` into the cli-runtime wire type owned by
/// `codex-cli-protocol`.
///
/// The types stay separate so cli-runtime protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `codex-config` into the cli-runtime wire type owned by
/// `codex-cli-protocol`.
///
/// The types stay separate so cli-runtime protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
