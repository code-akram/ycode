...
use chrono::DateTime;
use chrono::Utc;
use codex_cli_protocol::CurrentTimeReadParams;
use codex_cli_protocol::CurrentTimeReadResponse;
...
pub(crate) fn cli_runtime_time_provider(
    outgoing: Arc<OutgoingMessageSender>,
    thread_state_manager: ThreadStateManager,
) -> Arc<dyn TimeProvider> {
    Arc::new(CliRuntimeTimeProvider {
        outgoing: Arc::downgrade(&outgoing),
        thread_state_manager,
    })
}

struct CliRuntimeTimeProvider {
    outgoing: Weak<OutgoingMessageSender>,
    thread_state_manager: ThreadStateManager,
}

impl TimeProvider for CliRuntimeTimeProvider {
    fn current_time(&self, thread_id: ThreadId) -> TimeFuture<'_> {
        ...
        Box::pin(async move {
            let outgoing = outgoing
                .upgrade()
                .context("cli-runtime current-time provider is unavailable")?;
