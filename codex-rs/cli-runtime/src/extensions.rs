...
pub(crate) fn cli_runtime_extension_event_sink(
    outgoing: Arc<OutgoingMessageSender>,
    thread_state_manager: ThreadStateManager,
) -> Arc<dyn ExtensionEventSink> {
    Arc::new(CliRuntimeExtensionEventSink {
        outgoing,
...
struct CliRuntimeExtensionEventSink {
    outgoing: Arc<OutgoingMessageSender>,
    thread_state_manager: ThreadStateManager,
}
...
impl ExtensionEventSink for CliRuntimeExtensionEventSink {
...
mod tests {
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_uses_listener_fifo_for_goal_updates_warnings_and_clears() {
        ...
        let sink = cli_runtime_extension_event_sink(outgoing, thread_state_manager);
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_truncates_warning_before_listener_enqueue() {
        let (outgoing_tx, _outgoing_rx) = mpsc::channel(4);
        let outgoing = Arc::new(OutgoingMessageSender::new(
            outgoing_tx,
            AnalyticsEventsClient::disabled(),
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_targets_subscriber_without_listener() {
        ...
        let OutgoingMessage::CliRuntimeNotification(envelope) = message else {
            panic!("expected cli-runtime warning notification");
        };
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_waits_for_subscriber_without_listener() {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel(4);
        let outgoing = Arc::new(OutgoingMessageSender::new(
            outgoing_tx,
            AnalyticsEventsClient::disabled(),
        ...
        thread_state_manager
            ...
            .await;
        let sink = cli_runtime_extension_event_sink(outgoing, thread_state_manager.clone());
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_targets_subscriber_after_listener_closes() {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel(4);
        let outgoing = Arc::new(OutgoingMessageSender::new(
            outgoing_tx,
            AnalyticsEventsClient::disabled(),
    ...
    #[tokio::test]
    async fn cli_runtime_event_sink_drops_warning_with_invalid_thread_id() {
        let (outgoing_tx, mut outgoing_rx) = mpsc::channel(4);
        let outgoing = Arc::new(OutgoingMessageSender::new(
            outgoing_tx,
            AnalyticsEventsClient::disabled(),
        ));
        let sink = cli_runtime_extension_event_sink(outgoing, ThreadStateManager::new());
