...
use codex_cli_protocol::SelectedCapabilityRoot;
use codex_cli_protocol::ThreadSection;
use codex_cli_protocol::ThreadSectionMoveParams;
use codex_cli_protocol::ThreadSectionMoveResponse;
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        cli_runtime_client_name: Option<&str>,
    ) -> Result<Option<ClientResponsePayload>, JSONRPCErrorError> {
        if cli_runtime_client_name != Some(CODEX_TUI_CLIENT_NAME) {
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(codex_cli_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<codex_cli_protocol::ThreadStartSource>,
        ...
        let NewThread {
            ...
            .start_thread(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(codex_cli_protocol::ThreadStartSource::Startup)
                {
                    codex_cli_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    codex_cli_protocol::ThreadStartSource::Clear => InitialHistory::Cleared,
                },
                history_mode,
                thread_source,
                dynamic_tools,
            ...
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.create_thread",
                otel.name = "cli_runtime.thread_start.create_thread",
        ...
        let config_snapshot = thread
            .config_snapshot()
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.config_snapshot",
                otel.name = "cli_runtime.thread_start.config_snapshot",
            ))
            .await;
        ...
        log_listener_attach_result(
            ...
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.attach_listener",
                otel.name = "cli_runtime.thread_start.attach_listener",
        ...
        listener_task_context
            ...
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.upsert_thread",
                otel.name = "cli_runtime.thread_start.upsert_thread",
            ))
            .await;
        ...
        thread.status = resolve_thread_status(
            listener_task_context
                ...
                .instrument(tracing::info_span!(
                    "cli_runtime.thread_start.resolve_status",
                    otel.name = "cli_runtime.thread_start.resolve_status",
                ))
                .await,
        ...
        listener_task_context
            ...
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.send_response",
                otel.name = "cli_runtime.thread_start.send_response",
            ))
            .await;
        ...
        listener_task_context
            ...
            .instrument(tracing::info_span!(
                "cli_runtime.thread_start.notify_started",
    ...
    async fn thread_shell_command_inner(
        ...
        // `thread/shellCommand` is cli-runtime's local-host shell escape hatch,
    ...
    async fn apply_thread_read_store_fields(
        ...
        if include_turns {
            if matches!(
                thread.history_mode,
                codex_cli_protocol::ThreadHistoryMode::Paginated
            ) {
                self.thread_store
    ...
    async fn thread_turns_list_response_inner(
        ...
        let active_turn = if loaded_thread.is_some() {
            ...
            // cli-runtime listener has already projected live turn events into ThreadState,
    ...
    // cli-runtime-only hydration path until those clients use `thread/items/list`.
    ...
    ) -> Result<codex_cli_protocol::TurnsPage, JSONRPCErrorError> {
    ...
    pub(crate) async fn connection_closed(&self, connection_id: ConnectionId) {
        ...
        for thread_id in thread_ids {
            if self.thread_manager.get_thread(thread_id).await.is_err() {
                // Reconcile stale cli-runtime bookkeeping when the thread has already been
    ...
    async fn thread_resume_inner(
        ...
        let stored_thread_from_running_probe = match self
            .resume_running_thread(
                ...
                cli_runtime_client_version.clone(),
            )
            .await
        {
            ...
            .await
        {
            Ok(NewThread {
                ...
                if let Err(err) = Self::set_cli_runtime_client_info(
                    codex_thread.as_ref(),
                    cli_runtime_client_name,
                    cli_runtime_client_version,
                )
