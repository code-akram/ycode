...
) -> Vec<codex_cli_protocol::SkillMetadata> {
    skills
        .iter()
        .map(|skill| {
            let enabled = !disabled_paths.contains(&skill.path_to_skills_md);
            codex_cli_protocol::SkillMetadata {
                name: skill.name.clone(),
                description: skill.description.clone(),
                short_description: skill.short_description.clone(),
                interface: skill.interface.clone().map(|interface| {
                    codex_cli_protocol::SkillInterface {
                ...
                dependencies: skill.dependencies.clone().map(|dependencies| {
                    codex_cli_protocol::SkillDependencies {
                        tools: dependencies
                            .tools
                            .into_iter()
                            .map(|tool| codex_cli_protocol::SkillToolDependency {
                                r#type: tool.r#type,
                                value: tool.value,
...
) -> Vec<codex_cli_protocol::SkillErrorInfo> {
    errors
        .iter()
        .map(|err| codex_cli_protocol::SkillErrorInfo {
                ...
                async move {
                    let (cwd_abs, config_layer_stack) = match self.resolve_cwd_config(&cwd).await {
                        ...
                        Err(message) => {
                            ...
                            return (
                                index,
                                codex_cli_protocol::SkillsListEntry {
                                    cwd,
                                    skills: Vec::new(),
                                    errors: vec![codex_cli_protocol::SkillErrorInfo {
                                        path: error_path,
                                        message,
                                    }],
                                },
    ...
    async fn hooks_list_response(
        ...
        for cwd in cwds {
                ...
                .await
            {
                ...
                Err(err) => {
                    let error_path = cwd.clone();
                    data.push(codex_cli_protocol::HooksListEntry {
                        cwd,
                        hooks: Vec::new(),
                        warnings: Vec::new(),
