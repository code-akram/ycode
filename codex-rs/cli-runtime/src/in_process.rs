//! In-process cli-runtime runtime host for local embedders.
//!
...
//! channels. The intent is to preserve cli-runtime semantics while avoiding a
...
//! cli-runtime rather than creating a second execution contract.
//!
//! # Backpressure
//!
...
//!
//! # Relationship to `codex-cli-runtime-client`
//!
//! This module provides the low-level runtime handle ([`InProcessClientHandle`]).
//! Higher-level callers (TUI, exec) should go through `codex-cli-runtime-client`,
...
/// Input needed to start an in-process cli-runtime runtime.
///
...
pub struct InProcessStartArgs {
    ...
    /// Process-wide SQLite state handle shared with embedded cli-runtime consumers.
...
/// Event emitted from the cli-runtime to the in-process client.
///
...
/// Handle used by an in-process client to call cli-runtime and consume events.
///
/// This is the low-level runtime handle. Higher-level callers should usually go
/// through `codex-cli-runtime-client`, which adds worker-task buffering,
...
impl InProcessClientHandle {
    ...
    /// runtime event stream; sending arbitrary IDs has no effect on cli-runtime
...
/// Starts an in-process cli-runtime runtime and performs initialize handshake.
///
    ...
    let runtime_handle = tokio::spawn(async move {
                                ...
                                Err(mpsc::error::TrySendError::Full(_)) => {
                                        ...
                                        pending_request_responses.remove(&request_id)
                                    {
                                        let _ = response_tx.send(Err(JSONRPCErrorError {
                                            ...
                                            message: "in-process cli-runtime request queue is full"
                                                .to_string(),
                                            data: None,
                                        }));
                                    }
                                ...
                                Err(mpsc::error::TrySendError::Closed(_)) => {
                                        ...
                                        pending_request_responses.remove(&request_id)
                                    {
                                        let _ = response_tx.send(Err(internal_error(
                                            "in-process cli-runtime request processor is closed",
                                        )));
                                    }
                                    break;
                                }
        ...
        outgoing_message_sender
            .cancel_all_requests(Some(internal_error(
                "in-process cli-runtime runtime is shutting down",
            )))
            .await;
...
mod tests {
    ...
    #[tokio::test(start_paused = true)]
    async fn in_process_shutdown_waits_for_runtime_completion() {
