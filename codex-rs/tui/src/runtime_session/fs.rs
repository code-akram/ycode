...
impl CliRuntimeSession {
    ...
    pub(crate) async fn fs_read_file_path(&mut self, path: &CliRuntimePath) -> Result<Vec<u8>> {
    ...
    pub(crate) async fn fs_remove_path(&mut self, path: &CliRuntimePath) -> Result<()> {
    ...
    async fn request_fs_path<T: DeserializeOwned>(
        ...
        _remote_params: serde_json::Value,
    ) -> Result<T> {
        let request_id = self.next_request_id();
        match self.request_handle() {
            CliRuntimeRequestHandle::InProcess(_) => {
                let path = AbsolutePathBuf::from_absolute_path_checked(path.as_str())
                    .wrap_err_with(|| format!("invalid local cli-runtime fs path {path}"))?;
                self.client
