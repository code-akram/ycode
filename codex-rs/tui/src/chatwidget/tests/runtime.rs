...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> codex_cli_protocol::ThreadSettingsUpdatedNotification {
    codex_cli_protocol::ThreadSettingsUpdatedNotification {
...
async fn collab_spawn_end_shows_requested_model_and_effort() {
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            started_at_ms: 0,
            item: CliRuntimeThreadItem::CollabAgentToolCall {
                id: "call-spawn".to_string(),
                tool: CliRuntimeCollabAgentTool::SpawnAgent,
                status: CliRuntimeCollabAgentToolCallStatus::InProgress,
    ...
    chat.handle_server_notification(
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: CliRuntimeThreadItem::CollabAgentToolCall {
                id: "call-spawn".to_string(),
                tool: CliRuntimeCollabAgentTool::SpawnAgent,
                status: CliRuntimeCollabAgentToolCallStatus::Completed,
                sender_thread_id: sender_thread_id.to_string(),
                ...
                agents_states: HashMap::from([(
                    ...
                    CliRuntimeCollabAgentState {
                        status: CliRuntimeCollabAgentStatus::PendingInit,
                        message: None,
                    },
                )]),
            },
...
#[tokio::test]
async fn live_cli_runtime_user_message_item_completed_does_not_duplicate_rendered_prompt() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_user_message_omits_unsupported_media() {
        ...
        ServerNotification::ItemCompleted(ItemCompletedNotification {
            ...
            item: CliRuntimeThreadItem::UserMessage {
                ...
                content: vec![
                    ...
                    CliRuntimeUserInput::Audio {
                        url: "https://example.com/one.wav".to_string(),
                    },
                    CliRuntimeUserInput::LocalAudio {
    ...
    assert_chatwidget_snapshot!(
        "live_cli_runtime_user_message_omits_unsupported_media",
        lines_to_single_string(&inserted[0]),
    );
}

#[tokio::test]
async fn live_cli_runtime_turn_completed_clears_working_status_after_answer_item() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let item = CliRuntimeThreadItem::AgentMessage {
        id: "msg-1".to_string(),
...
#[tokio::test]
async fn live_cli_runtime_turn_started_sets_feedback_turn_id() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_warning_notification_renders_message() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_file_change_item_started_preserves_changes() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_command_execution_strips_shell_wrapper() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            turn_id: "turn-1".to_string(),
            started_at_ms: 0,
            item: CliRuntimeThreadItem::CommandExecution {
                id: "cmd-1".to_string(),
    ...
    assert_chatwidget_snapshot!(
        "live_cli_runtime_command_execution_strips_shell_wrapper",
        blob
    );
}

#[tokio::test]
async fn live_cli_runtime_command_output_delta_transcript_snapshot() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    for delta in ["stdout\n", "stderr\n"] {
        chat.handle_server_notification(
            ServerNotification::CommandExecutionOutputDelta(
                codex_cli_protocol::CommandExecutionOutputDeltaNotification {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
    ...
    assert_chatwidget_snapshot!("live_cli_runtime_command_output_delta_active", active);

    let transcript = chat
        .active_cell_transcript_lines(/*width*/ 80)
        .expect("active exec transcript lines");
    assert_chatwidget_snapshot!(
        "live_cli_runtime_command_output_delta_transcript",
    ...
    assert_chatwidget_snapshot!(
        "live_cli_runtime_command_output_delta_interrupted",
        completed
    );
}

#[tokio::test]
async fn live_cli_runtime_collab_wait_items_render_history() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: CliRuntimeThreadItem::CollabAgentToolCall {
                id: "wait-1".to_string(),
                tool: CliRuntimeCollabAgentTool::Wait,
                status: CliRuntimeCollabAgentToolCallStatus::InProgress,
                sender_thread_id: sender_thread_id.to_string(),
            ...
            item: CliRuntimeThreadItem::CollabAgentToolCall {
                ...
                agents_states: HashMap::from([
                    (
                        receiver_thread_id.to_string(),
                        CliRuntimeCollabAgentState {
                            status: CliRuntimeCollabAgentStatus::Completed,
                    ...
                    (
                        other_receiver_thread_id.to_string(),
                        CliRuntimeCollabAgentState {
                            status: CliRuntimeCollabAgentStatus::Running,
                            message: None,
                        },
                    ),
                ]),
    ...
    let combined = drain_insert_history(&mut rx)
        ...
        .collect::<Vec<_>>()
        .join("\n");
    assert_chatwidget_snapshot!("cli_runtime_collab_wait_items_render_history", combined);
}

#[tokio::test]
async fn live_cli_runtime_collab_spawn_completed_renders_requested_model_and_effort() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let sender_thread_id =
    ...
    let combined = drain_insert_history(&mut rx)
        ...
        .join("\n");
    assert_chatwidget_snapshot!(
        "cli_runtime_collab_spawn_completed_renders_requested_model_and_effort",
        combined
    );
}

#[tokio::test]
async fn live_cli_runtime_failed_turn_does_not_duplicate_error_history() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::TurnCompleted(TurnCompletedNotification {
            ...
            turn: CliRuntimeTurn {
                ...
                items_view: codex_cli_protocol::TurnItemsView::Full,
                items: Vec::new(),
                status: CliRuntimeTurnStatus::Failed,
                error: Some(CliRuntimeTurnError {
                    message: "permission denied".to_string(),
                    codex_error_info: None,
                    additional_details: None,
                }),
...
#[tokio::test]
async fn live_cli_runtime_failed_turn_consolidates_streamed_answer() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_turn_completion_repairs_dropped_message_deltas() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let mut completed_turn = cli_runtime_turn(
        "turn-1",
        CliRuntimeTurnStatus::Completed,
        Some(1_000),
        /*error*/ None,
    );
    completed_turn.items_view = codex_cli_protocol::TurnItemsView::Summary;
    completed_turn.items = vec![CliRuntimeThreadItem::AgentMessage {
        id: "msg-1".to_string(),
        text: concat!(
    ...
    let consolidations = std::iter::from_fn(|| rx.try_recv().ok())
        .filter_map(|event| match event {
            AppEvent::ConsolidateAgentMessage {
                ...
                assert_chatwidget_snapshot!(
                    "live_cli_runtime_turn_completion_repairs_dropped_message_deltas",
                    source,
                );
                Some(())
            }
...
#[tokio::test]
async fn live_cli_runtime_stream_recovery_restores_previous_status_header() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_server_overloaded_error_renders_warning() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn live_cli_runtime_cyber_policy_error_renders_dedicated_notice() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
...
#[tokio::test]
async fn cli_runtime_safety_access_errors_render_dedicated_notice() {
    ...
    insta::assert_snapshot!(
        "cli_runtime_bio_policy_error_renders_dedicated_notice",
        rendered_cases.last().unwrap().1.as_str()
    );
}

#[tokio::test]
async fn live_cli_runtime_model_verification_renders_warning() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.handle_server_notification(
        ServerNotification::ModelVerification(ModelVerificationNotification {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            verifications: vec![CliRuntimeModelVerification::TrustedAccessForCyber],
        }),
        /*replay_kind*/ None,
    );
...
#[tokio::test]
async fn live_cli_runtime_invalid_thread_name_update_is_ignored() {
...
#[tokio::test]
async fn live_cli_runtime_thread_name_update_shows_resume_hint() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let thread_id =
...
#[tokio::test]
async fn live_cli_runtime_thread_closed_requests_immediate_exit() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
