...
Repo-wide runs use the source-build path in `run.py`; targeted packaged runs can
still use `run-prebuilt-linter.py`.
...
`just argument-comment-lint` invokes the source wrapper. The Python wrappers
default the underlying Cargo invocation to `--all-targets` unless you explicitly
narrow the target set, so targeted wrapper runs cover test-only call sites by
default.
