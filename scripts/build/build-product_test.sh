...
write_fake_cargo() {
  ...
  printf '%s\n' \
    ...
    'printf "cargo|%s|%s|%s|%s\n" "$PWD" "$*" "${RUSTY_V8_ARCHIVE:-}" "${RUSTY_V8_SRC_BINDING_PATH:-}" >> "$BUILD_PRODUCT_TEST_LOG"' \
    >"$destination"
...
PATH="$TEST_PATH" \
  ...
  "$WRAPPER" build
grep -F "cargo|$WORKSPACE|build|$V8_CACHE/$ARCHIVE_NAME|$V8_CACHE/$BINDING_NAME" "$LOG" >/dev/null ||
